...
pub fn apply_generated_columns(
    ...
    let want_primzahlkreuz = should_generate_primzahlkreuz_pro_contra(generated_befehle);
    let want_love_polygon = should_generate_love_polygon(generated_befehle);
    let want_gleichheit_freiheit = should_generate_gleichheit_freiheit(generated_befehle);
    let want_geist_emotion = should_generate_geist_emotion_energie_materie_topologie(generated_befehle);
    let want_prim_creativity = should_generate_prim_creativity_type(generated_befehle);
    let want_mond_typ = should_generate_mond_exponzieren_logarithmus_typ(generated_befehle);
    let want_vervielfache = should_generate_vervielfache_zeile(generated_befehle);
    let want_modallogik = should_generate_modallogik(generated_befehle);

    if !(want_primzahlkreuz
        || want_love_polygon
        || want_gleichheit_freiheit
        || want_geist_emotion
        || want_prim_creativity
        || want_mond_typ
        || want_vervielfache
        || want_modallogik)
    {
    ...
    if want_modallogik {
        let concept_pairs = build_modal_concept_pairs(&table[0], generated_befehle);
        if !concept_pairs.is_empty() {
            concat_modallogik(&mut table, &concept_pairs, &mut rows_as_numbers, &mut tables);
        }
    }
    ...
    let selected_existing_cols = if want_vervielfache {
        collect_vervielfache_existing_cols(&rows_as_numbers)
    } else {
        Vec::new()
    };

    rebuild_output_with_selected_existing_and_generated(
        headers,
        data,
        &table,
        &selected_existing_cols,
        original_header_len,
    )?;
    Ok(())
}
...
fn contains_any(generated_befehle: &BTreeSet<String>, aliases: &[&str]) -> bool {
    let normalized: BTreeSet<String> = generated_befehle
        .iter()
        .map(|s| normalize_lookup_token(s))
        .collect();
...
fn contains_all_pairs(
    generated_befehle: &BTreeSet<String>,
    first_aliases: &[&str],
    second_aliases: &[&str],
) -> bool {
    contains_any(generated_befehle, first_aliases) && contains_any(generated_befehle, second_aliases)
}

// Die folgenden Regeln sind best-effort direkt aus words.py abgeleitet:
// paraNdataMatrix-Einträge der Form
//   (ParametersMain.<erstes_wort>, (<zweites_wort_aliase>...), ..., {"trigger"})
// sind eindeutige Concat-Trigger.
// Außerdem nutzen einige Concat-Funktionen keine explizite Trigger-Menge,
// sondern hängen indirekt an den Spaltenmengen eines paraNdataMatrix-Eintrags
// (z.B. Liebe -> Spalte 9, Freiheit/Gleichheit -> 132, Geist -> 242, Gestirn -> 64).
fn should_generate_primzahlkreuz_pro_contra(generated_befehle: &BTreeSet<String>) -> bool {
    contains_any(generated_befehle, &["primzahlkreuzprocontra"])
        || contains_all_pairs(
            generated_befehle,
            &["procontra", "dagegendafuer", "bedeutung"],
            &["primzahlkreuz", "primzahlkreuzprocontra", "Primzahlkreuz_pro_contra"],
        )
        || contains_all_pairs(
            generated_befehle,
            &["grundstrukturen"],
            &["nachvollziehen", "nachvollziehenemotionalodergeistigdurchprimzahlkreuzalgorithmus15"],
        )
}

fn should_generate_love_polygon(generated_befehle: &BTreeSet<String>) -> bool {
    contains_all_pairs(
        generated_befehle,
        &["menschliches"],
        &["liebe", "ethik", "Liebe"],
    ) || contains_any(generated_befehle, &["lovepolygon"])
}

fn should_generate_gleichheit_freiheit(generated_befehle: &BTreeSet<String>) -> bool {
    contains_all_pairs(
        generated_befehle,
        &["menschliches"],
        &["gleichheit", "freiheit"],
    ) || contains_any(generated_befehle, &["gleichheitfreiheit"])
}

fn should_generate_geist_emotion_energie_materie_topologie(
    generated_befehle: &BTreeSet<String>,
) -> bool {
    contains_all_pairs(
        generated_befehle,
...
fn should_generate_prim_creativity_type(generated_befehle: &BTreeSet<String>) -> bool {
    contains_all_pairs(
        generated_befehle,
        &["menschliches"],
        &[
            "evolutionaererwerbenundintelligenzkreativitaet",
            "evolutionaer",
            "evolutionär",
            "evolution",
            "intelligenz",
...
fn should_generate_mond_exponzieren_logarithmus_typ(generated_befehle: &BTreeSet<String>) -> bool {
    contains_all_pairs(
        generated_befehle,
        &["bedeutung"],
        &["gestirn", "mond", "sonne", "planet"],
...
fn should_generate_vervielfache_zeile(generated_befehle: &BTreeSet<String>) -> bool {
    contains_any(
        generated_befehle,
        &[
            "vielfache",
...
fn should_generate_modallogik(generated_befehle: &BTreeSet<String>) -> bool {
    contains_any(
        generated_befehle,
        &[
            "modal",
...
fn normalize_lookup_token(s: &str) -> String {
    s.to_lowercase()
        .replace('ä', "ae")
        .replace('ö', "oe")
        .replace('ü', "ue")
...
fn is_reserved_generated_befehl(token: &str) -> bool {
    should_generate_primzahlkreuz_pro_contra(&BTreeSet::from([token.to_string()]))
        || should_generate_love_polygon(&BTreeSet::from([token.to_string()]))
        || should_generate_gleichheit_freiheit(&BTreeSet::from([token.to_string()]))
        || should_generate_geist_emotion_energie_materie_topologie(&BTreeSet::from([token.to_string()]))
        || should_generate_prim_creativity_type(&BTreeSet::from([token.to_string()]))
        || should_generate_mond_exponzieren_logarithmus_typ(&BTreeSet::from([token.to_string()]))
        || should_generate_vervielfache_zeile(&BTreeSet::from([token.to_string()]))
        || should_generate_modallogik(&BTreeSet::from([token.to_string()]))
}

fn build_modal_concept_pairs(
    headers: &[String],
    generated_befehle: &BTreeSet<String>,
    ...
    for token in generated_befehle {
        if is_reserved_generated_befehl(token) {
            continue;
        }

        let token_norm = normalize_lookup_token(token);
        ...
        if let Some((idx, _)) = headers
            ...
            .enumerate()
            .find(|(_, header)| {
                let header_norm = normalize_lookup_token(header);
                header_norm.contains(&token_norm) || token_norm.contains(&header_norm)
            })
        {
    ...
    for chunk in matched_indices.chunks(2) {
        if let [a, b] = chunk {
            pairs.insert((*a, *b));
        }
    }
...
fn collect_vervielfache_existing_cols(rows_as_numbers: &RowSet) -> Vec<usize> {
    rows_as_numbers
        .iter()
        .copied()
        .filter(|idx| *idx == 19 || *idx == 90)
        .collect()
}

fn rebuild_output_with_selected_existing_and_generated(
    headers: &mut Vec<String>,
    data: &mut Vec<Vec<String>>,
    table: &Table,
    selected_existing_cols: &[usize],
    first_generated_col: usize,
) -> Result<(), Box<dyn std::error::Error>> {
    let header_row = table.first().ok_or("Leere Tabelle nach Generierung")?;

    let mut keep_cols: Vec<usize> = selected_existing_cols
        .iter()
        .copied()
        .filter(|idx| *idx < header_row.len())
        .collect();

    for idx in first_generated_col..header_row.len() {
        if !keep_cols.contains(&idx) {
            keep_cols.push(idx);
        }
    }
    ...
    *headers = keep_cols.iter().map(|idx| header_row[*idx].clone()).collect();
    *data = table
        .iter()
        .skip(1)
        .map(|row| {
...
fn keep_only_generated_columns(
    headers: &mut Vec<String>,
    data: &mut Vec<Vec<String>>,
    table: &Table,
    first_generated_col: usize,
    ...
    let header_row = table.first().ok_or("Leere Tabelle nach Generierung")?;
    if first_generated_col >= header_row.len() {
        return Ok(());
    }

    let generated_headers = header_row[first_generated_col..].to_vec();
    let generated_data: Vec<Vec<String>> = table
        .iter()
        .skip(1)
        .map(|row| {
            if row.len() < first_generated_col {
                Vec::new()
            } else {
                row[first_generated_col..].to_vec()
            }
        })
        .collect();
...
fn run_generator_into_headers_and_data<F>(
    headers: &mut Vec<String>,
    data: &mut Vec<Vec<String>>,
    _bereich: &TextBereich,
    generator_name: &str,
...
where
    F: FnOnce(&mut Table, &mut RowSet, &mut Tables),
{
    if headers.is_empty() {
        return Ok(());
    }
    ...
    generator(&mut table, &mut rows_as_numbers, &mut tables);

    if table.is_empty() {
        return Ok(());
    }

    let new_header_len = table[0].len();
    if new_header_len < original_header_len {
        return Err(format!("Generierte Spalten für {generator_name} haben die Tabellenbreite verkleinert").into());
    }

    keep_only_generated_columns(headers, data, &table, original_header_len)?;
    Ok(())
}

fn concat_love_polygon_into_headers_and_data(
    headers: &mut Vec<String>,
    data: &mut Vec<Vec<String>>,
    bereich: &TextBereich,
) -> Result<(), Box<dyn std::error::Error>> {
    run_generator_into_headers_and_data(headers, data, bereich, "love_polygon", |table, rows_as_numbers, tables| {
        concat_love_polygon(table, rows_as_numbers, tables);
    })
}

fn concat_gleichheit_freiheit_dominieren_into_headers_and_data(
    headers: &mut Vec<String>,
    data: &mut Vec<Vec<String>>,
    bereich: &TextBereich,
) -> Result<(), Box<dyn std::error::Error>> {
    run_generator_into_headers_and_data(headers, data, bereich, "gleichheit_freiheit_dominieren", |table, rows_as_numbers, tables| {
        concat_gleichheit_freiheit_dominieren(table, rows_as_numbers, tables);
    })
}

fn concat_geist_emotion_energie_materie_topologie_into_headers_and_data(
    headers: &mut Vec<String>,
    data: &mut Vec<Vec<String>>,
    bereich: &TextBereich,
) -> Result<(), Box<dyn std::error::Error>> {
    run_generator_into_headers_and_data(headers, data, bereich, "geist_emotion_energie_materie_topologie", |table, rows_as_numbers, tables| {
        concat_geist_emotion_energie_materie_topologie(table, rows_as_numbers, tables);
    })
}

fn concat_prim_creativity_type_into_headers_and_data(
    headers: &mut Vec<String>,
    data: &mut Vec<Vec<String>>,
    bereich: &TextBereich,
) -> Result<(), Box<dyn std::error::Error>> {
    run_generator_into_headers_and_data(headers, data, bereich, "prim_creativity_type", |table, rows_as_numbers, tables| {
        concat_prim_creativity_type(table, rows_as_numbers, tables);
    })
}

fn concat_mond_exponzieren_logarithmus_typ_into_headers_and_data(
    headers: &mut Vec<String>,
    data: &mut Vec<Vec<String>>,
    bereich: &TextBereich,
) -> Result<(), Box<dyn std::error::Error>> {
    run_generator_into_headers_and_data(headers, data, bereich, "mond_exponzieren_logarithmus_typ", |table, rows_as_numbers, tables| {
        concat_mond_exponzieren_logarithmus_typ(table, rows_as_numbers, tables);
    })
}
...
fn concat1_primzahlkreuz_pro_contra_into_headers_and_data(
    ...
    if !should_generate_primzahlkreuz_pro_contra(generated_befehle) {
        return Ok(());
    }
    ...
    let new_header_len = table[0].len();
    if new_header_len < original_header_len {
        return Err("Generierte Primzahlkreuz-Spalten haben die Tabellenbreite verkleinert".into());
    }

    *headers = table[0].clone();
    *data = table.into_iter().skip(1).collect();
