...
fn concat1_primzahlkreuz_pro_contra_into_headers_and_data(
    ...
    // Interne Arbeitsstruktur wie in der Generatorlogik:
    // Zeile 0 = Header, danach Datenzeilen.
    let mut table: Table = Vec::with_capacity(data.len() + 1);
    table.push(headers.clone());
    table.extend(data.iter().cloned());

    // Alle vorhandenen Spalten zunächst als "normale" Spalten markieren.
    let mut rows_as_numbers: RowSet = (0..original_header_len).collect();

    // Minimaler Tabellenzustand, den concat1_primzahlkreuz_pro_contra erwartet.
    ...
    // Echte Generatorlogik anwenden: hängt 2 Spalten an.
    ...
    // Zurück in headers/data schreiben
