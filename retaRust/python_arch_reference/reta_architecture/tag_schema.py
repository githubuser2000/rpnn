...
"""Explicit architecture owner for Reta's table-tag topology.

Stage 42 moves the actual ``ST`` enum and the large tag/reverse-tag tables out
of ``libs/lib4tables_Enum.py`` into ``reta_architecture``.  The legacy module
remains as a compatibility facade that re-exports the historical names.
"""
...
class TagSchemaBundle:
    """Explicit registry/facade for the table-tag schema and reverse mappings."""

    def snapshot(self) -> dict:
        return {
            "class": "TagSchemaBundle",
            "stage": 42,
            "legacy_owner": "libs/lib4tables_Enum.py",
            "enum": "ST",
            "counts": {
                "primary_tag_groups": len(tableTags),
                "primary_reverse_entries": len(tableTags2),
                "kombi_tag_groups": len(tableTags_kombiTable),
                "kombi_reverse_entries": len(tableTags2_kombiTable),
                "kombi2_tag_groups": len(tableTags_kombiTable2),
                "kombi2_reverse_entries": len(tableTags2_kombiTable2),
            },
            "tag_names": [tag.name for tag in ST],
        }

    @staticmethod
    def tags_for_column(column_number: int, kombi_table: int | None = None) -> frozenset[ST]:
        column_number = int(column_number)
        if kombi_table is None:
            return tableTags2.get(column_number, frozenset())
        if kombi_table == 0:
            return tableTags2_kombiTable.get(column_number, frozenset())
        if kombi_table == 1:
            return tableTags2_kombiTable2.get(column_number, frozenset())
        raise ValueError(f"Unknown kombi_table selector: {kombi_table}")

    @staticmethod
    def columns_for_tags(tags: frozenset[ST], kombi_table: int | None = None) -> set[int]:
        tags = frozenset(tags)
        if kombi_table is None:
            return set(tableTags.get(tags, set()))
        if kombi_table == 0:
            return set(tableTags_kombiTable.get(tags, set()))
        if kombi_table == 1:
            return set(tableTags_kombiTable2.get(tags, set()))
        raise ValueError(f"Unknown kombi_table selector: {kombi_table}")


def bootstrap_tag_schema() -> TagSchemaBundle:
    return TagSchemaBundle()
