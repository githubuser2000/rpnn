...
Stage 28 ergänzt die Stage-27-Kategorien/Funktoren/natürlichen Transformationen um eine explizite Kapselkarte.
...
- `MarkdownAuditSpec`
- Probe: `architecture-map-json`
- Probe: `architecture-diagram-md`

Die neue Karte beantwortet:

```text
Welche alte reta-Komponente wird welche neue Kapsel?
Was ist in was enthalten?
Welche Stage hat welche Kapsel erzeugt?
Wie laufen Topologie, Prägarben, Garben, Morphismen, Gluing, Kategorien, Funktoren und natürliche Transformationen zusammen?
```

Kurzbild:

```text
Legacy → Topologie → Prägarben → Garben → universelles Gluing → Tabellen-Sektion → Endofunktoren/Morphismen → Renderer → Parität
```

Stage 28 ändert kein CLI-/Runtime-Verhalten. Es macht die bereits entstandene Architektur als Gesamtkarte inspizierbar.
...
## What changed in Stage 29 – architecture contracts

Stage 29 adds `reta_architecture/architecture_contracts.py`. Stage 27 named categories, functors and natural transformations; Stage 28 drew the capsule map. Stage 29 turns those names and flows into explicit architecture contracts: commutative diagrams, capsule boundaries and refactor laws.
...
The key Stage-29 contract is:

```text
future extraction is valid only when the relevant topology/presheaf/sheaf/workflow/state/output/legacy diagram still commutes and the owning capsule boundary is preserved.
```

The new layer is metadata-only and makes no intended runtime or CLI behaviour change.

## Stage 30 – Witness matrix over contracts

Stage 30 adds `reta_architecture/architecture_witnesses.py`.

It builds on the Stage-27 category theory layer, the Stage-28 architecture map and the Stage-29 contract layer. The purpose is to connect symbolic architecture objects back to concrete repository witnesses:

- file/glob/symbolic anchor witnesses
- capsule slices from old reta owners to new architecture capsules
- diagram witnesses for the Stage-29 commutative diagrams
- naturality witnesses for the registered natural transformations
- refactor obligations for future stages
