...
## Stage 18 – Generated-Column- und Modal-Morphismen weiter herausgezogen

Stage 18 baut auf Stage 17 auf und verlagert weitere Domänenalgorithmen aus `libs/lib4tables_concat.py` in `reta_architecture/generated_columns.py`.

Die neue Generated-Columns-Schicht besitzt jetzt zusätzlich:

- `concat_vervielfache_zeile(...)`
- `concat_modallogik(...)`
- `concat_prim_universe_row(...)`

Damit sind die Legacy-Methoden `concatVervielfacheZeile(...)`, `concatModallogik(...)` und `concat1RowPrimUniverse2(...)` nur noch Wrapper. `reta_architecture/table_generation.py` ruft diese Operationen nun direkt über das `GeneratedColumnsBundle` auf.

`lib4tables_concat.py` wurde dadurch von ca. 2849 auf ca. 1395 Zeilen reduziert. Der Generator-Registry-Snapshot enthält jetzt 9 registrierte Generator-Morphismen.

Geprüft wurden Syntax, Architektur-Probes, ein echter `reta.py`-Smoke-Test und kleine Original-vs-Stage-18-Paritätsvergleiche für Shell und Markdown. Die vollständige große Command-Parity-Matrix bleibt weiterhin offen.

---

# Stage 18: Heavy Generated Columns und Meta-Column-Morphismen

Stage 18 setzt die Stage-17-Entflechtung von `libs/lib4tables_concat.py` fort und zieht die schwereren Generated-Column-Pfade in explizite Architekturschichten.

Die Generated-Column-Schicht `reta_architecture/generated_columns.py` besitzt jetzt neben den Stage-17-Morphismen auch:
...
Zusätzlich ist `reta_architecture/meta_columns.py` als eigene Schicht hinzugekommen. Sie besitzt die Meta-/Konkreta-/Theorie-/Abstrakt-Spaltenmorphismen und den Primzahlkreuz-Innen/Außen/Seitlich-Klassifikator. Damit werden Generated Columns und Meta Columns architektonisch getrennt, obwohl sie beide weiterhin kompatibel über die alte `Concat`-Klasse erreichbar bleiben.

`table_generation.py` ruft die herausgezogenen Generated-Column-Morphismen nun direkt über `GeneratedColumnsBundle` auf. `lib4tables_concat.py` ist dadurch stark geschrumpft und agiert zunehmend als Kompatibilitätsadapter.

Neue/erweiterte Probe-Kommandos:

```bash
python -S reta_architecture_probe_py.py generated-columns-json
python -S reta_architecture_probe_py.py table-generation-json
python -S reta_architecture_probe_py.py package-integrity-json
```
...
- `tests.test_command_parity`: OK
- `python -S -m unittest -q`: 35 Tests, OK
- `reta_architecture_probe_py.py generated-columns-json`: OK
- `reta_architecture_probe_py.py table-generation-json`: OK
- `reta_domain_probe_py.py pair-json Religionen Hinduismus`: OK
- Smoke-Test `Religionen/sternpolygon`: OK
- Stage-17-vs-Stage-18-Parität für `--grundstrukturen=nachvollziehen`: stdout/stderr byte-identisch
...
Der nächste Restblock liegt in der verbliebenen CSV-/Bruch-/`readConcatCsv(...)`-Logik.
