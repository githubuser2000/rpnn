...
/*
 * rrpl executes through libretaprompt_input.so, but libretaprompt_commands.so is
...
RETAPROMPT_USED static retaprompt_command_entrypoint_argv retaprompt_commands_abi_anchor =
    retaprompt_commands_run_rpl_argv;
...
int main(int argc, char **argv) {
    ...
    if (status != 0) return status;
    return retaprompt_input_run_rpl_argv((size_t)argc, (const char *const *)argv);
}
