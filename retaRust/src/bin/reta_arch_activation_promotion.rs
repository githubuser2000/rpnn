...
fn main() {
    ...
    let (policy, policy_from_cli) =
        reta_architecture::TableViewActivationPromotionPolicy::from_cli_args(
            &reta_args,
    ...
    match serde_json::to_string_pretty(&out) {
        ...
        Err(error) => {
            eprintln!("failed to serialize activation promotion report: {error}");
            std::process::exit(1);
        }
    }
}
