...
use crate::shared::exact_i18n::{I18nSubset, PyAtom};
...
pub fn storeParamtersForColumns(i18n: &I18nSubset, state: &mut ProgramState) {
    ...
    for parameterEntry in i18n.paraNdataMatrix.iter() {
        ...
        let (paraMainDict3, paraDict3, dataDicts3) = mergeParameterDicts(
        ...
        state.paraMainDict = paraMainDict3;
...
pub fn parametersToCommandsAndNumbers(state: &mut ProgramState, i18n: &I18nSubset) {
    storeParamtersForColumns(i18n, state);
    let mut lastParameterType: String = "".to_string();

    for token in state.argvWithoutProgram.clone() {
...
pub fn bringAllImportantBeginThings(state: &mut ProgramState, i18n: &I18nSubset) {
    if state.allImportantBeginThingsDone {
        return;
    }
    state.argvWithoutProgram = if state.argv.len() > 1 { state.argv[1..].to_vec() } else { vec![] };
    parametersToCommandsAndNumbers(state, i18n);
    state.allImportantBeginThingsDone = true;
}
