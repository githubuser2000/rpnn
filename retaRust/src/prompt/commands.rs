...
fn compile_normalized_control_command(tokens: &[String]) -> Option<PromptCommand> {
    match tokens {
        [single] => match single.as_str() {
            ...
            "HELP" | "help" | "hilfe" => Some(PromptCommand::PrintHelp),
...
fn compile_command_inner(input: &str, prompt_mode: PromptModus) -> Result<PromptCommand, String> {
    ...
    match trimmed {
        ...
        "HELP" | "help" | "hilfe" => return Ok(PromptCommand::PrintHelp),
...
pub fn help_text() -> String {
    [
        ...
        "",
        "Autocomplete:",
        "  Grauer Text             automatische Inline-Vorschläge wie Python complete_while_typing",
        "  Pfeil rechts            Inline-Vorschlag komplett übernehmen",
        "  Alt+Rechts/Ctrl+Rechts  nächstes Wort des Inline-Vorschlags übernehmen",
...
pub fn commands_text() -> String {
    let mut lines = vec![
        ...
        String::new(),
        "  HELP, help, hilfe, befehle, kurzbefehle".to_string(),
...
mod tests {
    ...
    fn one_letter_help_alias_is_control_after_python_alias_expansion() {
        ...
        let command = compile_command("HELP", PromptModus::Normal).unwrap();
        assert!(matches!(command, PromptCommand::PrintHelp));
    }
