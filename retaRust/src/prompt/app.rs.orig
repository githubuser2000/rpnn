...
use reedline::{
    ...
    ColumnarMenu, DefaultValidator, Emacs, KeyCode,
...
use super::history::{
    default_history_path, default_log_path, persistent_prompt_history_enabled,
...
fn run_interactive_loop(
    ...
    let mut editor = match newSession(
        &history_path,
        state.current_mode(),
        persistent_prompt_history_enabled(&state.program_name, state.logging_enabled),
        state.prompt_mode,
        &completion_runtime,
    ) {
        Ok(editor) => editor,
    ...
    loop {
        ...
        match editor.read_line(&prompt) {
            Ok(Signal::Success(buffer)) => {
                ...
                let persistent_history_enabled =
                    persistent_prompt_history_enabled(&state.program_name, state.logging_enabled);
                let history_line_scrubbed = promptInput(
                    state,
                    &input,
                    persistent_history_enabled.then_some(&history_path),
                    &log_path,
                );
...
fn newSession(
    ...
    let history = if persistent_history_enabled {
        PromptToolkitFileHistory::with_file(2_000, history_path.clone())
    } else {
