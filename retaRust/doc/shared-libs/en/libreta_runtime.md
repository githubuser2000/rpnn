# libreta_runtime.so — execution network and heavy Reta core carrier

## Programmer summary

`libreta_runtime.so` is the shared-library boundary for: **execution network and heavy Reta core carrier**.

This library carries the heavy non-interactive Reta core. libreta.so calls it through private reta_runtime_core_* symbols.
...
| Field | Value |
|---|---|
| Artifact | `target/<profile>/libreta_runtime.so` |
| Crate | `reta_runtime` |
| Rust source | `crates/reta_runtime/src/lib.rs` |
| C header | `crates/reta_runtime/include/reta_runtime.h` |
| Documentation | `doc/shared-libs/de/libreta_runtime.md` and `doc/shared-libs/en/libreta_runtime.md` |
...
network: tasks are nodes/edges; FIFO/LIFO/queues/stacks/semaphores order work deterministically.
...
Private RetaRuntimeCoreFfiResponse strings; libreta.so forwards/free maps them. Direct callers use reta_runtime_core_free_string.
