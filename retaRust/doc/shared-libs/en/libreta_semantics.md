# libreta_semantics.so — semantics, selection spaces, topology, and presheaf

## Programmer summary

`libreta_semantics.so` is the shared-library boundary for: **semantics, selection spaces, topology, and presheaf**.

This component groups semantic inventories, choice values, and the domain boundary between parsing and table/rendering.
...
| Field | Value |
|---|---|
| Artifact | `target/<profile>/libreta_semantics.so` |
| Crate | `reta_semantics` |
| Rust source | `crates/reta_semantics/src/lib.rs` |
| C header | `crates/reta_semantics/include/reta_semantics.h` |
| Documentation | `doc/shared-libs/de/libreta_semantics.md` and `doc/shared-libs/en/libreta_semantics.md` |
...
topology/presheaf: local parameter information is modeled as compatible sections over selection spaces.
...
JSON/value strings returned by semantic helpers; release with reta_semantics_free_string.
