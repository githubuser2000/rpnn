# libretaprompt_input.so — retaPrompt input, autocomplete, autosuggest, and history

## Programmer summary

`libretaprompt_input.so` is the shared-library boundary for: **retaPrompt input, autocomplete, autosuggest, and history**.

This library contains the interactive reedline prompt. Mid-cursor autocomplete and mid-cursor autosuggest live here in the shared library, not in the C launcher.
...
| Field | Value |
|---|---|
| Artifact | `target/<profile>/libretaprompt_input.so` |
| Crate | `retaprompt_input` |
| Rust source | `crates/retaprompt_input/src/lib.rs` |
| C header | `crates/retaprompt_input/include/retaprompt_input.h` |
| Documentation | `doc/shared-libs/de/libretaprompt_input.md` and `doc/shared-libs/en/libretaprompt_input.md` |
...
bidirectional channel: user state and prompt state are coupled through local cursor sections.
...
JSON strings returned by retaprompt_input_autosuggestion_at_cursor_json; release with retaprompt_input_free_string.

The cursor position for `retaprompt_input_autosuggestion_at_cursor_json` is a byte offset into a UTF-8 string. If it points into the middle of a codepoint, the library clamps it to the previous valid character boundary.
