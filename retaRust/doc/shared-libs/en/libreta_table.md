# libreta_table.so — tables, view state, width logic, and sheaf gluing

## Programmer summary

`libreta_table.so` is the shared-library boundary for: **tables, view state, width logic, and sheaf gluing**.

This component contains real width and table helper logic and is the target boundary for further materialization, layout, and view state.
...
| Field | Value |
|---|---|
| Artifact | `target/<profile>/libreta_table.so` |
| Crate | `reta_table` |
| Rust source | `crates/reta_table/src/lib.rs` |
| C header | `crates/reta_table/include/reta_table.h` |
| Documentation | `doc/shared-libs/de/libreta_table.md` and `doc/shared-libs/en/libreta_table.md` |
...
sheaf: compatible local column/row/parameter sections are glued into a global table.
...
Width JSON returned by table helpers; release with reta_table_free_string.
