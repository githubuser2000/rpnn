# libreta.so — public stable Reta C ABI facade

## Programmer summary

`libreta.so` is the shared-library boundary for: **public stable Reta C ABI facade**.

This library is intentionally thin. It preserves the known public ABI, delegates heavy engine work to libreta_runtime.so, and keeps the private core components visible through DT_NEEDED topology.
...
| Field | Value |
|---|---|
| Artifact | `target/<profile>/libreta.so` |
| Crate | `reta` |
| Rust source | `src/lib.rs` |
| C header | `include/reta.h` |
| Documentation | `doc/shared-libs/de/libreta.md` and `doc/shared-libs/en/libreta.md` |
| Build profile | `debug` or `release` via `./build.sh <profile>` |

## Direct consumers

- `rreta`
- `external C/C++ clients`
- `dynamic loaders using RETA_LIB_PATH`
...
universal property: all non-interactive frontends factor through the same canonical ABI entry point.
...
RetaFfiResponse stdout_text/stderr_text; returned strings must be released with reta_free_string.
