# libreta_arch.so — architecture metadata, category, morphism, and topology

## Programmer summary

`libreta_arch.so` is the shared-library boundary for: **architecture metadata, category, morphism, and topology**.

This component keeps architecture counts and snapshot metadata separate from runtime. It is a documentation and diagnostics boundary.
...
| Field | Value |
|---|---|
| Artifact | `target/<profile>/libreta_arch.so` |
| Crate | `reta_arch` |
| Rust source | `crates/reta_arch/src/lib.rs` |
| C header | `crates/reta_arch/include/reta_arch.h` |
| Documentation | `doc/shared-libs/de/libreta_arch.md` and `doc/shared-libs/en/libreta_arch.md` |
...
mathematical category: objects are states/requests, morphisms are transformations, natural transformations guard parity.
...
Only borrowed static strings and numeric counts are returned; no free function is needed.
