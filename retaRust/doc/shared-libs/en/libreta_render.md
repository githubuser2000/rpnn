# libreta_render.so — rendering functors, especially GrundStrukHtml

## Programmer summary

`libreta_render.so` is the shared-library boundary for: **rendering functors, especially GrundStrukHtml**.

This component is intentionally the direct target of rgrundStrukHtml. HTML rendering code lives in the .so, not in the executable.
...
| Field | Value |
|---|---|
| Artifact | `target/<profile>/libreta_render.so` |
| Crate | `reta_render` |
| Rust source | `crates/reta_render/src/lib.rs` |
| C header | `crates/reta_render/include/reta_render.h` |
| Documentation | `doc/shared-libs/de/libreta_render.md` and `doc/shared-libs/en/libreta_render.md` |
...
functor: the same semantic structure is mapped into concrete output forms such as HTML.
...
HTML strings returned by reta_render_grundstruk_html; release with reta_render_free_string.
