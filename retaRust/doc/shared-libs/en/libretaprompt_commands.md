# libretaprompt_commands.so — retaPrompt command side and command morphisms

## Programmer summary

`libretaprompt_commands.so` is the shared-library boundary for: **retaPrompt command side and command morphisms**.

rrpb uses only this library. rrp/rrpl/rrpe also keep it directly so the command side is not dropped by --as-needed.
...
| Field | Value |
|---|---|
| Artifact | `target/<profile>/libretaprompt_commands.so` |
| Crate | `retaprompt_commands` |
| Rust source | `crates/retaprompt_commands/src/lib.rs` |
| C header | `crates/retaprompt_commands/include/retaprompt_commands.h` |
| Documentation | `doc/shared-libs/de/libretaprompt_commands.md` and `doc/shared-libs/en/libretaprompt_commands.md` |
...
morphisms from prompt text to executable Reta commands; no line editor or suggestion UI.
...
No public string ownership in the header; command execution delegates to Reta and prints/returns exit status.
