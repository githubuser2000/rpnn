# libreta_runtime.so — Ausführungsnetzwerk und schwerer Reta-Core-Träger

## Kurzfassung für Programmierer

`libreta_runtime.so` ist die Shared-Library-Grenze für: **Ausführungsnetzwerk und schwerer Reta-Core-Träger**.

Diese Bibliothek trägt den schweren nicht-interaktiven Reta-Kern. libreta.so ruft sie über private reta_runtime_core_*-Symbole auf.
...
| Feld | Wert |
|---|---|
| Artefakt | `target/<profile>/libreta_runtime.so` |
| Crate | `reta_runtime` |
| Rust-Quelle | `crates/reta_runtime/src/lib.rs` |
| C-Header | `crates/reta_runtime/include/reta_runtime.h` |
| Dokumentation | `doc/shared-libs/de/libreta_runtime.md` und `doc/shared-libs/en/libreta_runtime.md` |
...
Netzwerk: Aufgaben sind Knoten/Kanten; FIFO/LIFO/Queues/Stacks/Semaphores ordnen Arbeit deterministisch.
...
- `reta_runtime_abi_role_de`
- `reta_runtime_abi_role_en`
- `reta_runtime_abi_math_de`
- `reta_runtime_abi_math_en`
- `reta_runtime_abi_manifest_json`
- `reta_runtime_core_run_and_print_from_env_ffi`
- `reta_runtime_core_abi_version`
...
Private RetaRuntimeCoreFfiResponse-Strings; libreta.so leitet sie weiter. Direkte Aufrufer nutzen reta_runtime_core_free_string.
...
```bash
readelf -d target/release/libreta_runtime.so
nm -D --defined-only target/release/libreta_runtime.so
```
