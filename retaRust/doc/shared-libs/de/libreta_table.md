# libreta_table.so — Tabellen, View-Zustand, Breitenlogik und Garben-Verklebung

## Kurzfassung für Programmierer

`libreta_table.so` ist die Shared-Library-Grenze für: **Tabellen, View-Zustand, Breitenlogik und Garben-Verklebung**.

Diese Komponente enthält reale Breiten- und Tabellenhilfslogik und ist der Zielort für weitere Materialisierung, Layout und View-State.
...
| Feld | Wert |
|---|---|
| Artefakt | `target/<profile>/libreta_table.so` |
| Crate | `reta_table` |
| Rust-Quelle | `crates/reta_table/src/lib.rs` |
| C-Header | `crates/reta_table/include/reta_table.h` |
| Dokumentation | `doc/shared-libs/de/libreta_table.md` und `doc/shared-libs/en/libreta_table.md` |
...
Garbe: kompatible lokale Spalten-/Zeilen-/Parametersektionen werden zu einer globalen Tabelle verklebt.
...
- `reta_table_abi_math_de`
- `reta_table_abi_math_en`
...
Breiten-JSON der Tabellenhelfer; mit reta_table_free_string freigeben.
...
```bash
readelf -d target/release/libreta_table.so
nm -D --defined-only target/release/libreta_table.so
```
