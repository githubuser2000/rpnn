# libreta_parse.so — Parsing, Tokenisierung und Input-Morphismen

## Kurzfassung für Programmierer

`libreta_parse.so` ist die Shared-Library-Grenze für: **Parsing, Tokenisierung und Input-Morphismen**.

Diese Komponente besitzt den Shell-/Prompt-Tokenizer und ist die saubere Grenze zwischen Textoberfläche und semantischer Interpretation.
...
| Feld | Wert |
|---|---|
| Artefakt | `target/<profile>/libreta_parse.so` |
| Crate | `reta_parse` |
| Rust-Quelle | `crates/reta_parse/src/lib.rs` |
| C-Header | `crates/reta_parse/include/reta_parse.h` |
| Dokumentation | `doc/shared-libs/de/libreta_parse.md` und `doc/shared-libs/en/libreta_parse.md` |
...
Morphismenfamilie von rohem Text in tokenisierte, kanonisierbare Anfragefragmente.
...
Token-JSON von reta_parse_shell_tokens_json; mit reta_parse_free_string freigeben.
