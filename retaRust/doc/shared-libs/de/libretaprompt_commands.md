# libretaprompt_commands.so — retaPrompt-Befehlsseite und Command-Morphismen

## Kurzfassung für Programmierer

`libretaprompt_commands.so` ist die Shared-Library-Grenze für: **retaPrompt-Befehlsseite und Command-Morphismen**.

rrpb verwendet nur diese Library. rrp/rrpl/rrpe halten sie zusätzlich direkt, damit die Command-Seite nicht durch --as-needed verschwindet.
...
| Feld | Wert |
|---|---|
| Artefakt | `target/<profile>/libretaprompt_commands.so` |
| Crate | `retaprompt_commands` |
| Rust-Quelle | `crates/retaprompt_commands/src/lib.rs` |
| C-Header | `crates/retaprompt_commands/include/retaprompt_commands.h` |
| Dokumentation | `doc/shared-libs/de/libretaprompt_commands.md` und `doc/shared-libs/en/libretaprompt_commands.md` |
...
- `libreta.so (dynamisch über libloading, RETA_LIB_PATH optional)`
...
Morphismen von Prompt-Text zu ausführbaren Reta-Kommandos; keine Zeileneditor- oder Suggest-UI.
...
Keine öffentliche String-Ownership im Header; Command-Ausführung delegiert an Reta und gibt Exitstatus zurück.
...
```bash
readelf -d target/release/libretaprompt_commands.so
nm -D --defined-only target/release/libretaprompt_commands.so
```
