# libreta_render.so — Rendering-Funktoren, insbesondere GrundStrukHtml

## Kurzfassung für Programmierer

`libreta_render.so` ist die Shared-Library-Grenze für: **Rendering-Funktoren, insbesondere GrundStrukHtml**.

Diese Komponente ist bewusst direktes Ziel von rgrundStrukHtml. Der HTML-Code liegt damit in der .so und nicht mehr im Executable.
...
| Feld | Wert |
|---|---|
| Artefakt | `target/<profile>/libreta_render.so` |
| Crate | `reta_render` |
| Rust-Quelle | `crates/reta_render/src/lib.rs` |
| C-Header | `crates/reta_render/include/reta_render.h` |
| Dokumentation | `doc/shared-libs/de/libreta_render.md` und `doc/shared-libs/en/libreta_render.md` |
...
Funktor: dieselbe semantische Struktur wird in konkrete Ausgabeformen wie HTML abgebildet.
...
- `reta_render_abi_math_de`
- `reta_render_abi_math_en`
...
HTML-Strings von reta_render_grundstruk_html; mit reta_render_free_string freigeben.
...
```bash
readelf -d target/release/libreta_render.so
nm -D --defined-only target/release/libreta_render.so
```
