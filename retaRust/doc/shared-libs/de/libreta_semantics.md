# libreta_semantics.so — Semantik, Auswahlräume, Topologie und Prägarbe

## Kurzfassung für Programmierer

`libreta_semantics.so` ist die Shared-Library-Grenze für: **Semantik, Auswahlräume, Topologie und Prägarbe**.

Diese Komponente bündelt semantische Inventare, Wahlwerte und die fachliche Grenzziehung zwischen Parsing und Tabelle/Rendering.
...
| Feld | Wert |
|---|---|
| Artefakt | `target/<profile>/libreta_semantics.so` |
| Crate | `reta_semantics` |
| Rust-Quelle | `crates/reta_semantics/src/lib.rs` |
| C-Header | `crates/reta_semantics/include/reta_semantics.h` |
| Dokumentation | `doc/shared-libs/de/libreta_semantics.md` und `doc/shared-libs/en/libreta_semantics.md` |
...
Topologie/Prägarbe: lokale Parameterinformationen werden als kompatible Sektionen über Auswahlräumen modelliert.
...
- `reta_semantics_abi_math_de`
- `reta_semantics_abi_math_en`
...
JSON-/Wert-Strings der Semantikfunktionen; mit reta_semantics_free_string freigeben.
...
```bash
readelf -d target/release/libreta_semantics.so
nm -D --defined-only target/release/libreta_semantics.so
```
