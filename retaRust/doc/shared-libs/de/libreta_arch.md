# libreta_arch.so — Architektur-Metadaten, Kategorie, Morphismus und Topologie

## Kurzfassung für Programmierer

`libreta_arch.so` ist die Shared-Library-Grenze für: **Architektur-Metadaten, Kategorie, Morphismus und Topologie**.

Diese Komponente hält Architekturzählungen und Snapshot-Metadaten von der Runtime getrennt. Sie ist Dokumentations- und Diagnose-Grenze.
...
| Feld | Wert |
|---|---|
| Artefakt | `target/<profile>/libreta_arch.so` |
| Crate | `reta_arch` |
| Rust-Quelle | `crates/reta_arch/src/lib.rs` |
| C-Header | `crates/reta_arch/include/reta_arch.h` |
| Dokumentation | `doc/shared-libs/de/libreta_arch.md` und `doc/shared-libs/en/libreta_arch.md` |
...
mathematische Kategorie: Objekte sind Zustände/Anfragen, Morphismen sind Transformationen, natürliche Transformationen sichern Parität.
...
- `reta_arch_abi_library_name`
- `reta_arch_abi_crate_name`
- `reta_arch_abi_role_de`
- `reta_arch_abi_role_en`
- `reta_arch_abi_math_de`
- `reta_arch_abi_math_en`
...
Es werden nur geliehene statische Strings und Zählwerte zurückgegeben; keine Free-Funktion nötig.
...
```bash
readelf -d target/release/libreta_arch.so
nm -D --defined-only target/release/libreta_arch.so
```
