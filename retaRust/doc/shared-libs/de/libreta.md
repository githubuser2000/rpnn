# libreta.so — öffentliche, stabile Reta-C-ABI-Fassade

## Kurzfassung für Programmierer

`libreta.so` ist die Shared-Library-Grenze für: **öffentliche, stabile Reta-C-ABI-Fassade**.

Diese Bibliothek soll klein bleiben. Sie hält die bekannte öffentliche ABI, delegiert aber die schwere Engine-Arbeit an libreta_runtime.so und hält die privaten Core-Komponenten als DT_NEEDED-Topologie sichtbar.
...
| Feld | Wert |
|---|---|
| Artefakt | `target/<profile>/libreta.so` |
| Crate | `reta` |
| Rust-Quelle | `src/lib.rs` |
| C-Header | `include/reta.h` |
| Dokumentation | `doc/shared-libs/de/libreta.md` und `doc/shared-libs/en/libreta.md` |
...
- `rreta`
- `externe C/C++-Clients`
- `dynamische Loader über RETA_LIB_PATH`
...
universelle Eigenschaft: alle nicht-interaktiven Frontends faktorisieren durch denselben kanonischen ABI-Einstieg.
...
RetaFfiResponse stdout_text/stderr_text; zurückgegebene Strings müssen mit reta_free_string freigegeben werden.
...
```bash
readelf -d target/release/libreta.so
nm -D --defined-only target/release/libreta.so
```
