...
// Strukturen für Tabellendaten mit integriertem Wortumbruch
#[derive(Debug, Clone)]
struct TableCell {
    lines: Vec<String>,
...
impl<'a> CliOutput<'a> {
    ...
    fn create_test_table(&self) -> Vec<TableRow> {
...
// Testfunktion für Wortumbruch
fn test_word_wrap() {
    println!("\n=== Test word_wrap() Funktion ===");
    ...
    for (text, width) in test_cases {
        ...
        for (i, line) in wrapped.iter().enumerate() {
            println!("  Zeile {}: '{}' (Länge: {})", i + 1, line, line.len());
        }
    }
}
...
fn test_colorize() {
    ...
    // Testsammlung - mit Referenz verwenden
    let tests = vec![
    ...
    for &(text, line_num, is_empty) in &tests {
        let colored = output.colorize(text, line_num, is_empty);
        println!("Line {} (empty: {}): {}", line_num, is_empty, colored);
    }
}

fn test_cli_out_plain_with_wrapping() {
    println!("\n=== Test cli_out() mit Wortumbruch - Plain Format ===");
    let tables = Tables::new(Some(100));
    ...
    let table_data = output.create_test_table();
    
    // Finde maximale Zeilenanzahl pro Zelle für rows_range
    let max_lines = table_data.iter()
        .map(|row| row.max_line_count())
        .max()
        .unwrap_or(1);
    
    let display_lines: BTreeSet<usize> = (0..table_data.len()).collect();
    let rows_range = 0..max_lines;
    
    println!("\n--- Tabellenausgabe mit Wortumbruch (Plain mit Farben) ---");
    println!("Maximale Zeilen pro Zelle: {}", max_lines);
    let result = output.cli_out(&display_lines, &table_data, rows_range);
    println!("\nGesamtanzahl ausgegebener Zeilen: {}", result.len());
}

fn test_cli_out_markdown_with_wrapping() {
    println!("\n=== Test cli_out() mit Wortumbruch - Markdown Format ===");
    let tables = Tables::new(Some(100));
    
    let mut output = CliOutput::new(&tables, OutputSyntax::Markdown);
    output.color_enabled = false;
    output.table_width = 70;
    output.line_numbering = true;
    ...
    println!("\n--- Tabellenausgabe mit Wortumbruch (Markdown) ---");
    let result = output.cli_out(&display_lines, &table_data, rows_range);
    println!("\nGesamtanzahl ausgegebener Zeilen: {}", result.len());
}
...
// Beispiel-Nutzung
fn main() {
    println!("=== TESTAUSGABEN FÜR TABELLEN-HANDLING MIT WORTUMBRUCH ===\n");
    
    // Teste Wortumbruch-Funktion
    test_word_wrap();
    
    // Teste einzelne Funktionen
    test_colorize();
    
    // Teste Tabellenausgabe mit Wortumbruch
    test_cli_out_plain_with_wrapping();
    test_cli_out_markdown_with_wrapping();
