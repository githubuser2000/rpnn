...
\title{3-adischer Steuerbaum mit Blockchain-Hash und Quellcode}
...
\lstdefinelanguage{C++}{
  keywords={int,double,float,if,else,for,while,struct,return,void,include},
...
\lstdefinelanguage{Java}{
  keywords={int,double,float,if,else,for,while,return,class,new,import,public,static,void},
  keywordstyle=\color{keywordcolor}\bfseries,
  comment=[l]{//},
  commentstyle=\color{commentcolor}\itshape,
  stringstyle=\color{stringcolor},
...
\lstdefinelanguage{Haskell}{
  keywords={data,deriving,let,in,map,if,then,else,return,do},
  keywordstyle=\color{keywordcolor}\bfseries,
  comment=[l]{--},
  commentstyle=\color{commentcolor}\itshape,
  stringstyle=\color{stringcolor},
...
\section{Implementierungen in C++, Java und Haskell}
...
class Node {
    ...
    double computeTax(){
        totalTax = localTax;
        for(Node c : children) totalTax += c.computeTax();
        return totalTax;
    }
}
\end{lstlisting}
...
data Node = Node { name :: String, localTax :: Double, children :: [Node] } deriving Show
totalTax :: Node -> Double
totalTax n = localTax n + sum (map totalTax (children n))
distributeEU :: Double -> [Node] -> [(String, Double)]
distributeEU tax countries =
    let share = tax / fromIntegral (length countries)
    in map (\c -> (name c, share)) countries
\end{lstlisting}
...
\begin{itemize}
    ...
    \item Implementierungen in C++, Java und Haskell vorliegen
\end{itemize}
