...
Dieses Dokument beschreibt die **Blocktree-Struktur**, die auf **Blockchain-Prinzipien basiert**, aber **baumförmig** ist.  
Ziel ist die Simulation von **Dorfsteuern**, deren Aggregation über **Bezirke → Bundesländer → Länder → EU-Länder** sowie **Verteilung der Transfers**.  

Wir betrachten **5-adische und 11-adische Strukturen**, diskutieren die Programmiersprachen **Idris2, Zig und Mojo**, und implementieren die Simulation in **C++, Java und Haskell**.

\section{Sprachvergleich: Idris2, Zig, Mojo}

\begin{itemize}
\item \textbf{Idris2}: Deklarativ, starke Typen, perfekt für **korrekte Blocktrees** und **Rekursion über Baumstrukturen**.  
\item \textbf{Zig}: Low-Level, maximal performant, imperativ. Gut für große Simulationen, aber verbose.  
\item \textbf{Mojo}: High-Performance, GPU/AI-orientiert, noch experimentell. Ideal für extrem große Parallel-Berechnungen.
\end{itemize}
...
Function GenerateNode(level, depth, parentHash):
    ...
    if level == "Dorf":
        ...
        children = []
        for 1..rand(1, branchingFactor): # 5 or 11
