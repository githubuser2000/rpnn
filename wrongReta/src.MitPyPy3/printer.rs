...
fn stretch_last_non_explicit_or_last_column(
    ...
    // Bei vollständig expliziten Breiten darf NICHT aufgefüllt werden.
    // Sonst macht `--breite 20` oder `--breiten 20,30,40` die letzte Spalte
    // wieder breiter als vom Benutzer verlangt.
...
fn stretch_last_column_to_fill_budget(
    widths: &mut [usize],
    chunk_budget: usize,
    ...
    match explicit_mask {
        Some(mask) => {
            if let Some(idx) = (0..widths.len())
                .rev()
                .find(|&i| !mask.get(i).copied().unwrap_or(false))
            {
                widths[idx] += extra;
            }
        }
...
pub fn print_table_chunked_with_line_numbers(
    ...
    while start < headers.len() {
        ...
        // Meta-Spalten (P/Z) sind implizit. Die Daten-Spalten können explizit sein.
        // Deshalb darf nur eine NICHT explizite Spalte Restbreite bekommen.
        let mut augmented_explicit_mask = vec![false; 2];
        augmented_explicit_mask.extend(explicit_mask_for_range(explizite_breiten, start, end));

        stretch_last_column_to_fill_budget(
            &mut augmented_widths,
            total_budget,
            Some(&augmented_explicit_mask),
        );
