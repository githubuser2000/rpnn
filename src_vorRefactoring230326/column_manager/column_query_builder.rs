...
use super::validation::validate_spalten_input_with_pair;

pub fn build_column_query(
    column_names: &[String],
    bereich: &mut TextBereich,
) -> Result<(String, Vec<String>), Box<dyn std::error::Error>> {
    let ober = column_names.get(0).cloned().unwrap_or_default();
    let unter = column_names.get(1).cloned().unwrap_or_default();

    validate_spalten_input_with_pair(
        bereich,
        &ober,
        &unter,
    )?;
